...
const SHEET_POSKAMLING = 'Data Poskamling';
const SHEET_POSYANDU = 'Data Posyandu';
...
const CPOS = { NAMA: 0, RTRW: 1, DESA: 2, KECAMATAN: 3, KOORDINAT: 4, LAT: 5, LNG: 6, FOTO: 7 };
...
// Simple reversible obfuscation — same pattern as SIMADUN
// Stored format: base64 of (reversed string)
function encodePass(pw) {
  if (!pw) return '';
  const rev = String(pw).split('').reverse().join('');
  return Buffer.from(rev, 'utf8').toString('base64');
}
...
function decodePass(encoded) {
  ...
  } catch (e) { return encoded; } // fallback for plain passwords
}
  ...
  try {
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      if (String(row[0] || '').toLowerCase() === username.toLowerCase()) {
        ...
        // Try encoded comparison first
        const storedDecoded = decodePass(stored);
        const match = storedDecoded === password || stored === password;
        if (match) {
          // If stored as plain text, upgrade to encoded
          if (stored === password && isEncoded(encodePass(password))) {
            try { await updateCell(SHEET_USERS, i + 1, 2, encodePass(password)); } catch (e2) { }
          }
          return success({
...
function getSheetInitDefinitions() {
  return [
    ...
    { name: SHEET_POSKAMLING, head: ['Nama Pos', 'RT/RW', 'Desa/Kelurahan', 'Kecamatan', 'Koordinat', 'Latitude', 'Longitude', 'Foto', 'Keterangan', 'Timestamp'], freeze: 1 },
    { name: SHEET_POSYANDU, head: ['Nama Posyandu', 'RT/RW', 'Desa/Kelurahan', 'Kecamatan', 'Koordinat', 'Latitude', 'Longitude', 'Foto', 'Keterangan', 'Timestamp'], freeze: 1 }
  ];
}
...
async function initAllSheets(payload) {
  try {
    ...
    if (!only || only === SHEET_USERS) {
      const users = await getSheetValues(SHEET_USERS);
      if (users.length < 2) {
        await appendRows(SHEET_USERS, [['admin', 'admin', 'admin', 'Administrator']]);
        summary.push('[INIT] User admin default (admin/admin).');
      }
    }
...
function posRow2obj(row, ri) {
  return {
    _ri: ri,
    nama: String(row[0] || '').trim(),
    rtrw: String(row[1] || '').trim(),
    desa: String(row[2] || '').trim(),
    kecamatan: String(row[3] || '').trim(),
    koordinat: String(row[4] || '').trim(),
    lat: String(row[5] || '').trim(),
    lng: String(row[6] || '').trim(),
    foto: String(row[7] || '').trim(),
    keterangan: String(row[8] || '').trim(),
    ts: String(row[9] || '').trim(),
  };
}

async function getPoskamlingData() {
  try {
    const values = await getSheetValues(SHEET_POSKAMLING);
    if (values.length < 2) return success({ data: [] });
    const data = values.slice(1)
      .map((row, i) => posRow2obj(row, i + 2))
      .filter(r => r.nama !== '');
    return success({ data });
  } catch (e) { return error(e.message); }
}

async function addPoskamling(payload) {
  try {
    if (!payload.nama || !String(payload.nama).trim()) return error('Nama Pos wajib diisi.');
    const ts = formatTsSheets(new Date());
    const row = [
      payload.nama || '', payload.rtrw || '', payload.desa || '', payload.kecamatan || '',
      payload.koordinat || '', payload.lat || '', payload.lng || '',
      payload.foto || '', payload.keterangan || '', ts
    ];
    await appendRows(SHEET_POSKAMLING, [row]);
    return success(null, 'Data Poskamling berhasil ditambahkan.');
  } catch (e) { return error(e.message); }
}

async function updatePoskamling(payload) {
  try {
    if (!payload._ri) return error('Row index tidak valid.');
    const ri = parseInt(payload._ri);
    const row = [
      payload.nama || '', payload.rtrw || '', payload.desa || '', payload.kecamatan || '',
      payload.koordinat || '', payload.lat || '', payload.lng || '',
      payload.foto || '', payload.keterangan || '', payload.ts || ''
    ];
    await updateRow(SHEET_POSKAMLING, ri, row);
    return success(null, 'Data Poskamling berhasil diperbarui.');
  } catch (e) { return error(e.message); }
}

async function deletePoskamling(ri) {
  try {
    await deleteRow(SHEET_POSKAMLING, parseInt(ri));
    return success(null, 'Data Poskamling berhasil dihapus.');
  } catch (e) { return error(e.message); }
}
...
async function getPosyanduData() {
  try {
    const values = await getSheetValues(SHEET_POSYANDU);
    if (values.length < 2) return success({ data: [] });
    const data = values.slice(1)
      .map((row, i) => posRow2obj(row, i + 2))
      .filter(r => r.nama !== '');
...
async function addPosyandu(payload) {
  try {
    if (!payload.nama || !String(payload.nama).trim()) return error('Nama Posyandu wajib diisi.');
    const ts = formatTsSheets(new Date());
    const row = [
      payload.nama || '', payload.rtrw || '', payload.desa || '', payload.kecamatan || '',
      payload.koordinat || '', payload.lat || '', payload.lng || '',
    ...
    await appendRows(SHEET_POSYANDU, [row]);
    return success(null, 'Data Posyandu berhasil ditambahkan.');
  } catch (e) { return error(e.message); }
}

async function updatePosyandu(payload) {
  try {
    if (!payload._ri) return error('Row index tidak valid.');
    const ri = parseInt(payload._ri);
    const row = [
    ...
    await updateRow(SHEET_POSYANDU, ri, row);
    return success(null, 'Data Posyandu berhasil diperbarui.');
  } catch (e) { return error(e.message); }
}

async function deletePosyandu(ri) {
  try {
    await deleteRow(SHEET_POSYANDU, parseInt(ri));
    return success(null, 'Data Posyandu berhasil dihapus.');
  } catch (e) { return error(e.message); }
}
