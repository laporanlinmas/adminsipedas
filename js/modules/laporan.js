...
function makeChipDesktop(identitas) {
  ...
  var html = '<div style="position:relative;display:inline-block;max-width:100%;">';
  html += '<div class="chip cr2" style="max-width:100px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:inline-block;vertical-align:middle;cursor:pointer;transition:all 0.2s;text-align:left;box-sizing:border-box;" title="Klik untuk rincian" ' +
    'onclick="var on=this.dataset.exp===\'1\'; document.querySelectorAll(\'.chip.cr2[data-exp=\\\'1\\\']\').forEach(function(el){if(el!==this){ el.dataset.exp=\'0\'; el.style.position=\'relative\'; el.style.zIndex=\'1\'; el.style.maxWidth=\'100px\'; el.style.width=\'auto\'; el.style.top=\'auto\'; el.style.left=\'auto\'; el.style.boxShadow=\'none\'; el.style.whiteSpace=\'nowrap\'; el.style.borderRadius=\'50px\'; el.style.padding=\'\'; el.style.overflow=\'hidden\'; el.children[0].style.display=\'inline\'; el.children[1].style.display=\'none\'; }}.bind(this)); this.dataset.exp=on?\'0\':\'1\'; ' +
    'if(!on){ ' +
    'this.style.position=\'absolute\'; this.style.zIndex=\'999\'; this.style.maxWidth=\'450px\'; this.style.width=\'max-content\'; ' +
    'this.style.top=\'-6px\'; this.style.left=\'-6px\'; this.style.boxShadow=\'0 4px 15px rgba(0,0,0,0.2)\'; ' +
    'this.style.whiteSpace=\'normal\'; this.style.borderRadius=\'8px\'; this.style.padding=\'10px 14px\'; this.style.overflow=\'hidden\'; ' +
    'this.children[0].style.display=\'none\'; this.children[1].style.display=\'block\'; ' +
    '} else { ' +
    'this.style.position=\'relative\'; this.style.zIndex=\'1\'; this.style.maxWidth=\'100px\'; this.style.width=\'auto\'; ' +
    'this.style.top=\'auto\'; this.style.left=\'auto\'; this.style.boxShadow=\'none\'; ' +
    'this.style.whiteSpace=\'nowrap\'; this.style.borderRadius=\'50px\'; this.style.padding=\'\'; this.style.overflow=\'hidden\'; ' +
    'this.children[0].style.display=\'inline\'; this.children[1].style.display=\'none\'; ' +
    '}">';
...
function buildRekapRows(sl, st, isAdm) {
  var rows = '', cards = '';
  if (!sl.length) {
    rows = '<tr><td colspan="13"><div class="empty"><i class="fas fa-inbox"></i><p>Tidak ada data</p></div></td></tr>';
    ...
    sl.forEach(function (r, i) {
      ...
      rows += '<tr>'
        ...
        + '<td class="tc-spt" style="font-family:var(--mono);font-size:.68rem">' + esc(r.noSpt || '') + '</td>'
      ...
      cards += '<div class="mcard-item">'
        ...
        + (r.noSpt ? '<i class="fas fa-hashtag" style="color:var(--purple);width:14px"></i> ' + esc(r.noSpt) + '<br>' : '')
  ...
  return _rData.filter(function (r) {
    if (_rFQ) {
      ...
      if ((r.lokasi || '').toLowerCase().indexOf(q) < 0
        && (r.noSpt || '').toLowerCase().indexOf(q) < 0
...
function openPdf(row) {
  ...
  loadPdfSettings(function () {
    ...
    var ns = G('pdf-nospt'); if (ns) ns.value = row.noSpt || '';
...
function openEditModal(row) {
  ...
  G('medit-body').innerHTML = ''
    ...
    + '<div class="fcol"><label class="flbl">No SPT</label><input class="fctl" id="ed-nospt" value="' + esc(row.noSpt || '') + '"></div>'
