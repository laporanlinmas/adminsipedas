...
function loadPeta() {
  ...
  _petaMode = 'leaflet'; // FIX: Reset state so switchPetaMode works later
  ...
  var h = '<div class="fu peta-container" id="peta-main-wrap" style="padding:0!important;position:relative">'
    +'<div style="padding:10px 14px 0;flex-shrink:0;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">'
      +'<div class="peta-mode-toggle">'
        ...
        +(PETA_MYMAPS_URL?'<button class="peta-mode-btn" id="btn-mymaps" onclick="switchPetaMode(\'mymaps\')"><i class="fas fa-map"></i> Google My Maps</button>':'')
      +'</div>'
      ...
      +'<div id="mymaps-wrap" style="height:100%;position:relative;border-radius:12px;overflow:hidden;border:1px solid var(--border);box-shadow:var(--sh);display:none">'
        +(PETA_MYMAPS_URL
          ? '<div class="peta-map-shimmer" id="peta-shimmer"><div class="peta-map-shimmer-ico"><i class="fas fa-map-location-dot"></i></div><p>Memuat peta interaktif...</p></div>'
            + '<div style="position:absolute;inset:0;overflow:hidden;border-radius:12px"><iframe id="peta-frame" src="'+escUrlAttr(PETA_MYMAPS_URL)+'" allowfullscreen="" loading="eager" referrerpolicy="no-referrer-when-downgrade" onload="onPetaFrameLoad()" style="position:absolute;inset:0;width:100%;height:100%;border:none;display:block;opacity:0;transition:opacity .4s"></iframe></div>'
          : '<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:20px;text-align:center;font-size:.72rem;color:var(--muted);line-height:1.5"><div><i class="fas fa-map" style="font-size:1.6rem;opacity:.25;display:block;margin-bottom:10px"></i>URL embed Google My Maps belum diatur.<br>Set <code style="font-size:.65rem">PETA_EMBED_URL</code> di <code style="font-size:.65rem">__SIPEDAS_ENV__</code></div></div>'
        )
      +'</div>'
    +'</div>'
...
function reloadPetaFrame(){
  if(!PETA_MYMAPS_URL){ toast('URL embed My Maps belum diatur.','er'); return; }
  ...
  setTimeout(function(){fr.src=PETA_MYMAPS_URL;},100);
}

function switchPetaMode(mode){
  if(_petaMode===mode) return;
  if(mode==='mymaps' && !PETA_MYMAPS_URL){
    toast('URL embed Google My Maps belum diatur (PETA_EMBED_URL).','er');
    return;
  }
  _petaMode=mode;
  var btnMM=G('btn-mymaps'),btnLF=G('btn-leaflet');
  var wMM=G('mymaps-wrap'),wLF=G('leaflet-wrap');
  var bEL=G('btn-edit-layer'),bPR=G('btn-print-pdf');
  if(mode==='mymaps'){
